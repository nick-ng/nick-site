...
exports.up = function(knex) {
    return Promise.all([
        ...
        .then(() =>
            knex.schema.createTable(mediaTable, table => {
                table.increments('id').primary();
                table.string('uri', 1000).unique();
                table
                    .integer('type_id')
                    .references('id')
                    .inTable(mediaTypeTable)
                    .onDelete('CASCADE');
            })
        )
        .then(() =>
            knex.schema.createTable(mediaCategoryRelationTable, table => {
                table.increments('id').primary();
                table
                    .integer('media_id')
                    .references('id')
...
exports.down = function(knex) {
    return knex.schema
        .dropTableIfExists(mediaCategoryRelationTable)
