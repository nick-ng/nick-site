...
import CubeastImporter from './cubeast';
...
const importCSTimerForeign = async (selectedSessions, convertedTimes) => {
  const relevantSessions = getRelevantSessions(selectedSessions, 'foreign***');
...
const importCSTimerLocal = (selectedSessions, convertedTimes) => {
  ...
  relevantSessions.forEach((sessionName) => {
    const oldHistory = JSON.parse(localStorage.getItem(sessionName) || '[]');
    const newHistory = oldHistory
      .concat(convertedTimes)
      .sort((a, b) => {
        const dateA = new Date(a.createdAt);
...
const SessionImporter = ({ selectedSessions, refreshHack }) => {
  ...
  return (
    <Container>
        ...
        return (
          <Button
            ...
            onClick={async () => {
              ...
              await importCSTimerForeign(selectedSessions, convertedTimes);
              importCSTimerLocal(selectedSessions, convertedTimes);
              refreshHack();
            }}
          >
            {sessionName}
      ...
      <Heading>Cubeast</Heading>
      <CubeastImporter
        importHandler={async (times) => {
          await importCSTimerForeign(selectedSessions, times);
          importCSTimerLocal(selectedSessions, times);
          refreshHack();
        }}
      />
    </Container>
